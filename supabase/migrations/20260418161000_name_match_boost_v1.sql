create or replace function public.search_name_match_v1(
  p_query text,
...
as $$
  with normalized as (
    select
      ...
      regexp_replace(lower(trim(coalesce(p_candidate_name, ''))), '[^a-z0-9]+', ' ', 'g') as candidate_text
  ),
  tokens as (
    select
      query_text,
  ...
  select case
    when query_text = '' or candidate_text = '' then 0::double precision
    when query_text = candidate_text then 1::double precision
    when candidate_text like '%' || query_text || '%' or query_text like '%' || candidate_text || '%' then 0.92::double precision
    when cardinality(query_tokens) >= 2
      and not exists (
        select 1
        from unnest(query_tokens) token
        where candidate_text not like '%' || token || '%'
      )
      then 0.88::double precision
    when cardinality(query_tokens) = 1
      and query_tokens[1] <> ''
      and candidate_text like '%' || query_tokens[1] || '%'
...
grant execute on function public.search_name_match_v1(text, text) to authenticated;
...
candidate_scores as (
  ...
  group by fc.candidate_id, fc.name, fc.skills, fc.years_experience, fc.primary_role, fc.role_tags, fc.current_title, fc.headline, fc.seniority
),
scored as (
  select
    fc.tenant_id,
    ...
    (
      (0.44 * cs.name_match)
      + (0.34 * cs.max_chunk_rrf)
      + (0.12 * cs.avg_top3_chunk_rrf)
      + (0.04 * cs.role_match)
      + (0.02 * cs.seniority_match)
      + (0.02 * cs.skill_match)
      + (0.02 * cs.experience_match)
    ) as score,
    jsonb_build_object(

...
export async function getAuthContext(supabase: SupabaseClient) {
  ...
  const [membershipResult, platformAdminResult] = await Promise.all([
    supabase.from("tenant_memberships").select("tenant_id, role, status").eq("user_id", user.id).eq("status", "active"),
  ...
  if (adminError && !/platform_admins/i.test(adminError.message) && adminError.code !== "PGRST205") {
    throw adminError;
  }

  type MembershipRow = { tenant_id: string; role: string; status: string };
  type TenantRow = { id: string; slug: string; name: string; icon_url: string | null };
  ...
  const tenantQuery = supabase.from("tenants").select("id, slug, name, icon_url");
  ...
  if (isPlatformAdmin) {
    memberships.push(...tenantRows.map((t) => mapTenant(t, "platform-admin", "active")));
  }

...
import { buildCompanyExclusionTerms, excludeCompanyMatches } from "./searchIntent.ts";
import { normalizeLocationValue, normalizeSkillList } from "./searchTaxonomy.ts";
...
export async function getManatalSyncStatus(
  ...
  return {
    ...
    totals: { sourceDocuments, gcsOriginals, driveOriginals, manatalRows, mappedManatalRows, syncedRows, pendingRows, failedRows, skippedRows },
    coverage: {
...
function statusForAlerts(alerts: OpsHealthRow[]) {
  if (alerts.some((alert) => alert.severity === "P0" || alert.severity === "P1")) return "degraded";
...
export async function getSystemHealth(
  ...
  const [snapshotResult, eventsResult, workersResult] = await Promise.all([
    supabase.rpc("ops_health_snapshot_v1", { p_tenant_ids: tenantIds.length ? tenantIds : null }),
    (() => {
      let query = supabase
  ...
  const workerRows = (workersResult.data ?? []) as Array<{ tenant_id: string; device_name: string; status: string; last_seen_at: string | null; metadata_json: unknown }>;
  ...
  const services = [
    { name: "Edge Functions", status: statusForAlerts(edgeAlerts), latency: latencyMs ? `${latencyMs} ms p95` : "no samples", detail: edgeAlerts[0]?.message ?? `${recentEvents.length} recent requests, ${eventsWithFailures} server errors.` },
    { name: "Search", status: statusForAlerts(searchAlerts), latency: latencyMs ? `${latencyMs} ms p95` : "no samples", detail: searchAlerts[0]?.message ?? "Search alerts are clear from the Supabase health snapshot." },
    { name: "Offline worker fleet", status: statusForAlerts(workerAlerts), latency: workerRows.length ? `${workerRows.length} registered` : "idle", detail: workerAlerts[0]?.message ?? (workerRows.length ? "Active worker devices are sending heartbeats." : "No worker devices registered; worker monitoring is idle.") },
    { name: "Ingestion", status: statusForAlerts(ingestionAlerts), latency: ingestionAlerts[0]?.current_value !== null && ingestionAlerts[0]?.current_value !== undefined ? `${ingestionAlerts[0].current_value}` : "clear", detail: ingestionAlerts[0]?.message ?? "No stuck or failing ingestion runs in the current alert window." },
    { name: "Data quality", status: statusForAlerts(dataQualityAlerts), latency: dataQualityAlerts[0]?.current_value !== null && dataQualityAlerts[0]?.current_value !== undefined ? `${dataQualityAlerts[0].current_value}%` : "clear", detail: dataQualityAlerts[0]?.message ?? "Recent parsing quality is within configured thresholds." },
    { name: "Supabase capacity", status: statusForAlerts(capacityAlerts), latency: capacityUsage ? `${Math.round(capacityUsage)}%` : "within limits", detail: capacityAlerts[0]?.message ?? "Database and storage capacity alerts are clear." },
  ];
  ...
  return {
    ...
    services,
    workerFleet,
    logs: alertLogs.length ? alertLogs : [{ level: "ok", message: "Supabase monitoring snapshot is clear.", timestamp: new Date().toLocaleTimeString("en-US", { hour12: false }) }],
  };
}
...
export async function getInsightsDashboard(
  ...
  await supabase
    ...
    .insert({
      ...
      filters: { tenant_ids: tenantIds, top_skills: topSkills, target_skills: targetSkills, target_role: targetRole },
...
export async function getInsightsGapAnalysis(
  ...
  await supabase
    ...
    .insert({
      ...
      filters: { tenant_ids: tenantIds, target_skills: targetSkills, target_role: targetRole },
      duration_ms: durationMs,
      row_count: null,
      trace_id: traceId,
    })
...
const insightReportRunSelect = [
  "id", "tenant_id", "initiated_by_user_id", "status", "report_type", "input_config",
  "report_payload", "failure_reason", "llm_provider", "llm_model", "started_at",
...
export const insightReportSchema = {
  ...
  properties: {
    title: { type: "string" },
    executiveSummary: { type: "string", description: "Two to four sentence executive summary grounded in provided metrics." },
    sections: {
      type: "array",
      items: {
        type: "object",
        properties: {
          ...
          citations: {
            ...
            items: {
              type: "object",
              properties: { metricKey: { type: "string" }, label: { type: "string" }, value: { type: "string" } },
    ...
    assistantPrompts: { type: "array", description: "Suggested follow-up questions or actions for the recruiter.", items: { type: "string" } },
  },
  required: ["title", "executiveSummary", "sections", "recommendations", "risks", "assistantPrompts"],
};

function normalizeInsightReportType(value: unknown): InsightReportType | null {
  const normalized = String(value ?? "").trim().toLowerCase();
  return normalized === "corpus_overview" || normalized === "gap_brief" || normalized === "job_family_analysis" ? normalized : null;
}
...
function distributionTop(items: JsonRecord[], limit = 3) {
  return items
    ...
    .map((item) => {
      ...
      return percent != null ? `${label} (${Math.round(value)} / ${Math.round(percent)}%)` : `${label} (${Math.round(value)})`;
    })
    .join(", ");
}
...
export function buildHeuristicInsightReport(
  ...
  const jobFamilies = asArray(snapshot.job_families ?? snapshot.jobFamilies).map((item) => asRecord(item));
  const seniority = asArray(snapshot.profiles_by_seniority ?? snapshot.profilesBySeniority).map((item) => asRecord(item));
  const locations = asArray(snapshot.profiles_by_location ?? snapshot.profilesByLocation).map((item) => asRecord(item));
  ...
  const targetSkills = asStringArray(gapAnalysis.target_skills ?? gapAnalysis.targetSkills);
  const fullyMatching = asNumber(gapAnalysis.fully_matching_candidates ?? gapAnalysis.fullyMatchingCandidates) ?? 0;
  const partiallyMatching = asNumber(gapAnalysis.partially_matching_candidates ?? gapAnalysis.partiallyMatchingCandidates) ?? 0;
  const missingSkills = asArray(gapAnalysis.missing_skills ?? gapAnalysis.missingSkills).map((item) => asRecord(item)).slice(0, 3).map((item) => asString(item.skill)).filter(Boolean);

  const focusLabel = focus?.trim() || targetRole || "the indexed corpus";
  const title = reportType === "gap_brief" ? `Gap brief: ${focusLabel}` : reportType === "job_family_analysis" ? `Job family analysis: ${focusLabel}` : "Corpus intelligence brief";
  ...
  const sections: InsightReportPayload["sections"] = [
    {
      ...
      citations: [
        { metricKey: "total_cvs_indexed", label: "Total CVs indexed", value: totalCvs },
        { metricKey: "avg_skills_per_profile", label: "Average skills per profile", value: avgSkills },
      ],
    },
    {
  ...
  if (reportType !== "corpus_overview") {
    sections.push({
      ...
      citations: [
        { metricKey: "fully_matching_candidates", label: "Fully matching candidates", value: String(fullyMatching) },
        { metricKey: "partially_matching_candidates", label: "Partially matching candidates", value: String(partiallyMatching) },
      ],
    });
  }
  ...
  const recommendations = reportType === "gap_brief"
    ? [
      partiallyMatching > fullyMatching && missingSkills[0] ? `Prioritize upskilling around ${missingSkills[0]} before expanding search criteria.` : "Run a targeted search for fully matching profiles before widening requirements.",
  ...
  const risks = [
    Number(totalCvs) <= 0 ? "Corpus volume is too low for reliable supply conclusions." : null,
    missingSkills.length && reportType === "gap_brief" ? `${missingSkills[0]} appears as a recurring blocker in partial profiles.` : null,
  ...
  return { title, executiveSummary, sections, recommendations, risks, assistantPrompts };
}
...
export async function startInsightReportRun(
  ...
  const reportType = normalizeInsightReportType(body.report_type ?? body.reportType);
  if (!reportType) {
    throw new Error("report_type must be corpus_overview, gap_brief, or job_family_analysis.");
  }
  ...
  const inputConfig = {
    ...
    targetRole: focus,
    targetSkills,
    topSkills: Math.max(1, Math.min(200, Math.trunc(asNumber(body.top_skills ?? body.topSkills) ?? 50))),
  };
  ...
  try {
    ...
    const generation = await generateInsightReportPayload(snapshot, reportType, focus);
    ...
    await writeAuditEvent(supabase, {
      ...
      entityType: "insight_report_run",
      entityId: runId,
      payload: { reportType, provider: generation.provider, model: generation.model },
    });

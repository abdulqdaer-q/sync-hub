...
export async function fetchSearchIntentFacets(
  ...
  const rows: Array<{ skills: string[] | null; companies: string[] | null; location: string | null }> = [];
  ...
  const excludedCompanyTerms = await fetchTenantCompanyExclusionTerms(supabase, tenantIds);
...
const GENERIC_TITLE_QUERY_TOKENS = new Set([
  "candidate", "developer", "development", "dev", "engineer", "engineering",
  "expert", "junior", "lead", "manager", "mid", "person", "people",
  "principal", "role", "senior", "software", "specialist", "staff",
]);
...
function roleSearchAliases(role: string | null | undefined) {
  switch (role) {
    case "frontend": return ["frontend", "front end", "front-end", "web developer", "web application engineer", "ui developer"];
    case "backend": return ["backend", "back end", "back-end", "api developer", "server developer", "server engineer"];
    case "full-stack": return ["full-stack", "full stack", "fullstack"];
    case "mobile": return ["mobile", "android", "ios", "flutter", "react native"];
    case "devops": return ["devops", "sre", "kubernetes", "terraform", "platform"];
    case "data": return ["data", "analytics", "etl", "bi"];
    case "ml": return ["ml", "ai", "machine learning", "llm"];
    case "qa": return ["qa", "quality", "test", "automation"];
    case "security": return ["security", "cybersecurity", "soc"];
...
function roleSkillAliases(role: string | null | undefined) {
  switch (role) {
    case "frontend": return ["react", "angular", "vue", "next.js", "nextjs", "javascript", "typescript", "html", "css", "tailwind", "bootstrap"];
    case "backend": return ["node.js", "node", "django", "flask", ".net", "asp.net", "java", "php", "laravel", "api", "rest api", "postgresql", "mysql"];
    case "full-stack": return ["react", "angular", "vue", "node.js", "node", ".net", "django", "laravel", "javascript", "typescript"];
    case "mobile": return ["android", "ios", "flutter", "react native", "swift", "kotlin", "dart"];
    case "devops": return ["kubernetes", "terraform", "docker", "aws", "azure", "ci/cd"];
    case "data": return ["data analysis", "analytics", "etl", "bi", "sql", "python", "pandas"];
    case "ml": return ["machine learning", "ml", "ai", "llm", "tensorflow", "pytorch"];
    case "qa": return ["qa", "quality assurance", "testing", "automation testing", "selenium"];
    case "security": return ["security", "cybersecurity", "soc", "penetration testing"];
    default: return [];
  }
}
...
function roleCompatibilityScore(row: CandidateSearchRow, role: string | null | undefined) {
  ...
  if (primaryRole === "full stack" && (normalizedRole === "frontend" || normalizedRole === "backend")) return 0.78;
  if (normalizedRole === "full stack" && primaryRole === "full stack") return 0.78;
  return 0;
}

function aliasHitCount(text: string, aliases: string[]) {
  return new Set(
    aliases.map(normalizeSearchText).filter((alias) => alias && text.includes(alias))
  ).size;
}
...
function genericEngineeringTitleScore(row: CandidateSearchRow, role: string | null | undefined) {
  ...
  if (role === "frontend" && /\b(?:software|ui|front end|frontend)\b/.test(title)) return 0.66;
  if (role === "backend" && /\b(?:software|backend|back end|api|server)\b/.test(title)) return 0.66;
  return 0;
}
...
function queryTextMatchScore(query: string, value: unknown) {
  ...
  if (candidateText.includes(queryText) || queryText.includes(candidateText)) return 0.92;
  if (queryTokens.length >= 2 && queryTokens.every((token) => candidateText.includes(token))) return 0.88;
  if (queryTokens.length === 1 && candidateText.includes(queryTokens[0])) return 0.72;
  return 0;
}
...
function contactMatchScore(row: CandidateSearchRow, query: string) {
  ...
  if (phoneDigits && queryDigits.length >= 5 && (phoneDigits === queryDigits || phoneDigits.includes(queryDigits) || queryDigits.includes(phoneDigits))) {
    return phoneDigits === queryDigits ? 1 : 0.92;
  }
  if (email && queryTokens.some((token) => email.includes(token))) return 0.88;
  if (phoneDigits && queryDigits.length >= 4 && phoneDigits.includes(queryDigits)) return 0.88;
  return 0;
}
...
function rowPassesExplicitFilters(row: CandidateSearchRow, filters: SearchIntentPayload) {
  ...
  if (filters.seniority && normalizeSearchText(row.seniority) !== normalizeSearchText(filters.seniority)) return false;
  if (filters.min_years_experience !== null && toFiniteNumber(row.years_experience) < filters.min_years_experience) return false;
  if (filterLocation && rowLocation !== filterLocation && !normalizeSearchText(row.location).includes(normalizeSearchText(filters.location))) return false;
  if (filters.skills.length && skillMatchScore(row, filters.skills) <= 0) return false;
  if (filters.companies.length && companyMatchScore(row, filters.companies) < 1) return false;
  return true;
}
...
export async function runFastProfileSearch(
  ...
  let scored = rows
    .filter((row) => !hasExcludedCompanyMatch(row.companies, excludedCompanyTerms))
  ...
  return scored
    ...
    .map((item) => mapFastProfileResult(item.row, item.score, query, filters, rankVersion));
}
